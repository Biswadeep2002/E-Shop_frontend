  ...
  const AddProductForm = ({ setOpen, product, update = false }) => {
    ...
    const { categoryLoader, errorMessge } = useSelector((state) => state.errors);
    ...
    if(categoryLoader) return <SkeletonAnimation/>
    if(errorMessge) return <ErrorPage message={errorMessge}/>
        ...
        <form className='space-y-4'
          ...
          <div className='flex flex-col gap-4 w-full'>
            ...
            <textarea
              ...
              className={`px-4 py-2 border w-full outline-hidden bg-transparent text-slate-800 rounded-md ${errors["description"]?.message ? "border-red-500" : "border-slate-700"
                }`}
                maxLength={255}
